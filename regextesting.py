...
# print(extracted_info)
# import re
text = "[marvelring] 마블링 프라임 11GB+매일2GB+3Mbps  | 22,950원 | 모요, 모두의요금제홈요금제 찾기인터넷 찾기휴대폰 찾기이벤트마이페이지홈요금제 찾기인터넷 찾기이벤트마이페이지모요개통월 11GB + 매일 2GB데이터 다 써도 저화질 영상 재생 가능 (3mbps)무제한무제한LG U+망LTE 35,714명이 선택35,714명이 선택잘못된 정보 제보모요개통 요금제만의 특별한 혜택쿠폰 받고 요금제 개통하면 100% 지급 쿠폰 이용 안내3,000원 쿠폰 받기잘못된 정보 제보월 22,950원7개월 이후 38,900원신청하기지금 신청하면 언제 개통될까요?유심을 사오면유심을 사오면 오늘 바로 개통할 수 있어요평균 25분 내 개통 완료유심 사러가기요금제 상세 정보 요금제 이름마블링 | 마블링 프라임 11GB+매일2GB+3Mbps통신사 약정없음펼쳐보기통신사 리뷰4.21,718개고객센터4.2개통 과정4.2개통 후 만족도4.3김*영31일 전엘지 원 칩 구매하니 몇분만에 개통이 되어서 좋네요. 다르느 통신사로 옮겨도 엘지 원칩을 그대로 사용할수도 있고. 통화품질도 굿~~~. 가성비 좋아 계속 쭉~이용할겁니다~정*태11일 전개통 빠르고 고객센터 소통 잘되고 뭐가 더 필요할가요이*우12일 전간편해서 좋습니다더보기요금제 개통 절차쓰던 번호로 개통할 때새 번호로 개통할 때1. 가입 신청원하는 요금제를 찾았다면 신청 버튼을 눌러 신청서를 작성해주세요. 가입 신청을 해도 기존에 사용하던 통신사는 바로 해지되지 않아요2. 정보 확인 및 유심 배송서류를 검토한 뒤 정보가 다 올바르면 유심을 발송해요. 올바르지 않은 정보가 있었다면 통신사에서 연락을 드릴 수 있어요3. 유심 받은 후 개통 진행유심을 받으셨다면 통신사 안내에 따라 개통 요청을 해주세요. 개통이 완료되면 기존에 쓰던 통신사는 이때 자동으로 해지돼요4. 새 유심으로 갈아끼면 끝기존 통신사가 해지되고 새로운 알뜰폰 유심으로 교체하면 알뜰폰 요금제 사용이 시작돼요"
pattern = r"월\s*\d+\s*GB\s*\+.*?(매일\s*[.\d]+(?:GB|MB))"
...
plan_18452 = "[sugarmobile] ITEMMANIA 5G 통화무제한 150GB++ | 49,000원 | 모요, 모두의요금제홈요금제 찾기인터넷 찾기휴대폰 찾기이벤트마이페이지홈요금제 찾기인터넷 찾기이벤트마이페이지모요개통월 150GB데이터 다 써도 고화질 영상 재생 가능 (5mbps)무제한무제한LG U+망5G 94명이 선택94명이 선택잘못된 정보 제보모요개통 요금제만의 특별혜택쿠폰 받고 요금제 개통하면 100% 지급쿠폰 이용 안내3,000원 쿠폰 받기잘못된 정보 제보월 49,000원신청하기꼭 확인해 주세요해당 요금제는 번호이동 시 이전통신사 90일 이상 사용하신 경우에만 가입 가능하며, 신규가입의 경우 통신사 내부 기준 (회선수, 사용이력)에 따라 가입제한이 있을 수 있습니다.개통 당월 해지하실 경우에는 할인이 적용되지 않고 기본월액으로 일할계산되어 요금이 부과됩니다.개통 가능한 시간을 알려드려요유심을 사오면유심을 사오면 오전 10시에 개통할 수 있어요신규 가입은 지금 개통할 수 있어요유심 사고 스타벅스 커피 1잔도 받아보세요유심 미리사기최대 53만원 지급 이벤트LG U+ 인터넷 신청하기요금제 상세 정보 요금제 이름슈가모바일 | ITEMMANIA 5G 통화무제한 150GB++통신사 약정없음통화무제한문자무제한통신망LG U+망통신 기술5G데이터 제공량월 150GB데이터 소진시5mbps 속도로 무제한부가통화300분번호이동 수수료없음일반 유심 배송무료NFC 유심 배송지원 안 함eSIM지원 안 함지원인터넷 결합모바일 핫스팟10GB 제공해외 로밍신청은 통신사에 문의미지원소액 결제데이터 쉐어링기본 제공 초과 시영상 통화3.3원/초부가 통화1.98원/초긴 문자33원/개사진 포함 긴 문자220원/개영상 포함 긴 문자220원/개통화 또는 문자 제공량이 무제한이더라도 과도한 사용이 있을 경우 사용량 제한이 있을 수 있어요.접기통신사 리뷰4.54,430개고객센터4.4개통 과정4.7개통 후 만족도4.5김람20일 전통신사들 너무 비싸서 비싼만큼 혜택은 못받는 거 같아서 옮겼어요. 알았다면 좀 더 일찍 옮겼을 거에요. 속도나, 이용면에서 절대 뒤떨어지지 않네요.김철35일 전슈가모바일 앱이 없는 것만 제외하고 매우 만족합니다. 이것저것 다 써봤는데, 신경 덜쓰고 3만원중반에 데이터 쾌적하게 사용하니까 정말 편안합니다. 한*영39일 전와이파이 되는 곳에서 번호이동 진행하세요. 통산사 이동되면서 가입하다 갑자기 기좀 통신사가 바로 연결 끊겨 잠시 딩황했네요 ㅋ 간편하게 번호이동~ 만족합니다^^더보기요금제 개통 절차쓰던 번호로 개통할 때새 번호로 개통할 때1. 가입 신청원하는 요금제를 찾았다면 신청 버튼을 눌러 신청서를 작성해주세요. 가입 신청을 해도 기존에 사용하던 통신사는 바로 해지되지 않아요2. 정보 확인 및 유심 배송서류를 검토한 뒤 정보가 다 올바르면 유심을 발송해요. 올바르지 않은 정보가 있었다면 통신사에서 연락을 드릴 수 있어요3. 유심 받은 후 개통 진행유심을 받으셨다면 통신사 안내에 따라 개통 요청을 해주세요. 개통이 완료되면 기존에 쓰던 통신사는 이때 자동으로 해지돼요4. 새 유심으로 갈아끼면 끝기존 통신사가 해지되고 새로운 알뜰폰 유심으로 교체하면 알뜰폰 요금제 사용이 시작돼요"